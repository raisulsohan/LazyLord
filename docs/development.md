...
- **Illustrator and Photoshop effects:** `PageItem.applyEffect` with the `Adobe PSL Gaussian Blur` and `Adobe Drop Shadow` LiveEffect XML (the Dict keys, that `blur` is a Gaussian radius, and that `I csrc 1` with `I dark 100` draws black); Photoshop's `newPlacedLayer` keeping the layer's place and opacity, `GsnB` on a smart object adding a smart filter, and the `setd` Lefx descriptor for Drop and Inner Shadow. That Figma's layer blur radius is twice a Gaussian radius is taken from its CSS export.
